...
export function typingWords(level) {
  if (level <= 3) return ["あ", "い", "う", "え", "お"];
  if (level <= 6) return ["かき", "くけこ", "さし", "すせそ", "あか"];
  if (level <= 10)
    return ["さかな", "ねこ", "いぬ", "はな", "やま", "くるま", "そら"];
  if (level <= 13)
    return ["はる", "まなび", "ともだち", "たから", "かもめ", "ひかり"];
  if (level <= 16)
    return [...FISH.slice(0, 12), ...DINOS.slice(0, 3)].map((x) => x.kana);
  if (level <= 18)
    return [
      "しゃしん",
      "きっぷ",
